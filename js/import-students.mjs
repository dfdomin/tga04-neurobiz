...
export function parseAcademusoftStudents(rawText) {
  ...
  for (const line of lines) {
    ...
    if (documentMatch) {
      students.push({
        name: cleanSpaces(documentMatch[3]),
        cc: documentMatch[2],
        grupo: meta.grupo,
    ...
    if (legacyMatch && !/\b(CC|TI|CE|PA|RC|NUIP)\b/i.test(legacyMatch[1])) {
      students.push({
        name: cleanSpaces(legacyMatch[1]),
        cc: legacyMatch[2],
        grupo: meta.grupo,
        horario: meta.horario,
      });
...
export function extractPdfTextFromContent(content) {
  const items = [];
  ...
  if (!items.length) {
    return [...content.matchAll(/\(((?:\\.|[^\\()])*)\)\s*Tj/g)].map(m => decodePdfString(m[1])).join("\n");
  }
...
export async function extractPdfTextFromArrayBuffer(buffer) {
  ...
  while (true) {
    ...
    if (dictionary.includes("FlateDecode")) {
      ...
      try {
        ...
        // Image streams or unsupported encodings can fail; keep scanning content streams.
      }
    }
...
export async function extractTextFromStudentFile(file) {
