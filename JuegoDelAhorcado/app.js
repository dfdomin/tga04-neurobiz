const STORAGE_STUDENTS = "tga04_students_v1";
const alphabet = "ABCDEFGHIJKLMNÑOPQRSTUVWXYZ".split("");
const bodyOrder = ["head", "torso", "left-arm", "right-arm", "left-leg", "right-leg"];

const WORD_BANK = {
  "NeuroBiz": [
    "NEUROBIZ", "ALGORITMO", "DATOS", "PYTHON", "VARIABLE", "FUNCION",
    "BUCLE", "CONDICIONAL", "LISTA", "DICCIONARIO", "API", "CLOUD",
    "STARTUP", "NEGOCIO", "TECNOLOGIA", "INNOVACION", "DIGITAL",
  ],
  "Computacion": [
    "COMPUTADOR", "SOFTWARE", "HARDWARE", "INTERNET", "NAVEGADOR",
    "ARCHIVO", "CARPETA", "MEMORIA", "PROCESADOR", "RED", "SERVIDOR",
    "EXCEL", "HOJA DE CALCULO", "FORMULA", "CELDA",
  ],
  "Programacion": [
    "CODIGO", "DEBUG", "EJECUTAR", "SINTAXIS", "ENTRADA", "SALIDA",
    "STRING", "INTEGER", "BOOLEAN", "IMPORT", "PRINT", "LOGICA",
  ],
  "Negocios": [
    "CLIENTE", "VENTAS", "MERCADO", "PRODUCTO", "SERVICIO", "ROI",
    "KPI", "ANALITICA", "AUTOMATIZACION", "PROCESO", "DECISION",
    "ESTRATEGIA", "EMPRESA", "EQUIPO", "PROYECTO",
  ],
};
...
function supabaseCredentials() {
  ...
  return {
    url: window.SUPABASE_URL || localStorage.getItem("tga04_supabase_url"),
    key: window.SUPABASE_KEY || localStorage.getItem("tga04_supabase_key"),
  };
}
...
async function fetchStudentsFromSupabase() {
  ...
  const offering = encodeURIComponent(
    (typeof GamifSDK !== "undefined" && GamifSDK.getOfferingCode()) || "TGA04-2026-2"
  );
  try {
    let res = await fetch(
